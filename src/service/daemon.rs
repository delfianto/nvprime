use crate::common::{nvgpu::NvGpu, config::{GpuTune, SysTune}};
...
impl DaemonState {
    ...
    pub fn init_gpu(&mut self, gpu_uuid: Option<String>) -> Result<()> {
        ...
        let mut gpu = NvGpu::init(gpu_uuid)
        ...
        self.baseline_power_limit = Some(device.power_management_limit_default()
...
mod tests {
    ...
    fn test_apply_gpu_tuning_no_gpu_initialized() {
        ...
        assert!(result.unwrap_err().to_string().contains("GPU not initialized"));
    }
